...
//    砂時計
// -----------
final double HOURGLASS_HEIGHT = 2.0;  
               // 砂時計の高さ (m)
...
final double SAND_GRAIN_MASS = 1.e-3;                    
               // 砂粒一つの質量 (kg)
final double SAND_GRAIN_DIAMETER = 2.e-2;                    
               // 砂粒の直径 (m) 
...
class Energy
{
  ...
  double getSpringPotential()
  {
    ...
    for (int i=0; i<NSGIP-1; i++) {
      double pos_y = grains[i].pos_y;  
      double distance_between_two_grains = grains[i+1].pos_y - pos_y;
      double ell = distance_between_two_grains 
                 - VIRTUAL_SPRING_NATURAL_LENGTH;
      positive_check( distance_between_two_grains, 
                     "distance_between_two_grains < 0?" );
...
//    double  y = SIMULATION_REGION_Y_MIN*0.5 + separation*i;
    double  y = floor.level_y + 1.0 + separation*i;
    double vy = 0.0; 
...
void draw_sand_grains_and_floors()
{
  ...
  // --<plot data>--
      double h =  grains[0].pos_y - floor.level_y;
      if ( sim.time > 0.445 && sim.time < 0.455 ) {
        if ( sim.nstep%10 == 0 ) {
          file.println("t, y= " + sim.time + " " + h);
        }
      } else {      
        if ( sim.nstep%100 == 0 ) {
          file.println("t, y= " + sim.time + " " + h);
