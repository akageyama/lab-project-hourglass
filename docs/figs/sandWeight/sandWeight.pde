...
boolean automatic_start = true;

final double INITIAL_RELAXATION_TIME = 2.0; 

final int NSTEP_AVERAGE_ELEMENT = 100;
final int NSTEP_AVERAGE = NSTEP_AVERAGE_ELEMENT * NSTEP_AVERAGE_ELEMENT;
...
final int    PARAM_VIEW_SPEED = 10000;   
...
class Simulation
{
  ...
  Simulation() 
  {
    time = -INITIAL_RELAXATION_TIME;  // シミュレーションの時刻 (s) 
...
//    size_for_work_array = (int)Math.round(Math.sqrt(PARAM_AVERAGE_TIME_SPAN_HINT/dt));
    size_for_work_array = NSTEP_AVERAGE_ELEMENT;
...
void setup() 
{
  ...
  file = createWriter("sand_weight.txt");  
}
...
void draw() 
{
  ...
  if ( sim.time >= PARAM_HOURGLASS_TIME_IN_SECOND + 2*INITIAL_RELAXATION_TIME ) {
    finish();
  }
...
void draw_sand_grains_and_floorLowers()
{
  ...
  if ( RunningStateToggle ) {
    for (int n=0; n<PARAM_VIEW_SPEED; n++) { // to speed up the display
      ...
      if ( sim.nstep % NSTEP_AVERAGE == 0 ) {
        file.println("t, w= " + sim.time + " " 
                     + analyser.average.str_avrg_of_array_of_avrg_of_array_of_rawdata);
      }      
