...
$M_0=1.0$~\si{(kg)},
$T_0=10.0$~\si{(s)}とした。
$t<0$にシミュレーションを開始し、十分緩和してから$t=0$から砂の落下を開始する。
時間刻み幅は $\Delta t = 1.9958\times 10^{-6}$~\si{(s)} である。
砂時計の重さの変化の理論値は
$\Delta W=2M_0 L_0/T_0^2 = 2.039\times 10^{-3}$~\si{(kg)}である。


抗力の変動が大きいので、
左のグラフでは1万ステップ毎の平均値、右のグラフは24万ステップ毎の平均値をとっている。
右のグラフからシミュレーションで得られた重さの増加は理論とよく一致していることがわかる。
